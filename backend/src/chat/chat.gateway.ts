...
export class ChatGateway implements OnModuleInit {
  ...
  async handleCreateGame(@ConnectedSocket() client: Socket, @MessageBody() channelId: string): Promise<any> {
    const userId = client.handshake.auth.userId;
    if (userId) {
      try {
  ...
  async handleJoinGame(@ConnectedSocket() client: Socket, @MessageBody() acceptingUserId: number): Promise<any> {
    const userId = client.handshake.auth.userId;
    if (userId) {
